...
CITE and FPKI Partner repositories should be available during regular business hours for scheduled and unscheduled testing. The CITE repository services operated by the FPKIMA are internet accessible, operational, and may be available up to 24 hours a day, 7 days a week with the exception of scheduled downtime, off hours, and federal holidays. The FPKI Partners will be notified in advance to the extent possible of any downtime and its scheduled duration. Any downtime notices will be posted to the System Notification page{:target="_blank"}.
...
{% include alert-warning.html content="FPKIMA and FPKI Partner technical support is only available for scheduled testing with operational any outage resolved on a best effort basis. " %}
