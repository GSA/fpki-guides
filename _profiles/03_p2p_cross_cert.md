...
| Subject   |       |       |   Unique X.500 Issuing CA DN.<BR>For CAs operating under the Common Policy CP, must use one of the name forms specified in the Common Policy CP, Section 3.1.1.<BR>Subject name should be encoded exactly as it is encoded in the Issuer field of certificates issued by the subject.<br>PrintableString encoding should be used wherever possible for Issuer and Subject Distinguished Names.    |
