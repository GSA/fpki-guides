...
Certificate profile formats and semantics are defined in X.509**<Footnote for X.509?>** and further described in RFC 5280.<sup>[1](#1)</sup> These Federal Public Key Infrastructure (FPKI) profiles specify the unique, detailed parameter settings for X.509 v3 certificates and v2 certificate revocation lists (CRLs) issued under the _X.509 Certificate Policy For The U.S. Federal PKI Common Policy Framework_ (aka, _Common Policy CP_). They also give guidance for those issued under other Certificate Policies that are cross-certified with the Federal Bridge Certification Authority (FBCA). Any FPKI profile deviations from the RFC 5280-specified formats are noted.
